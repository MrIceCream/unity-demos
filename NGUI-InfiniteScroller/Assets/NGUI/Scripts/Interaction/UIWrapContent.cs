...
public class UIWrapContent : MonoBehaviour
{
    ...
    public int[] ItemSizeArray = { 100, 200, 300, 100, 200, 300, 100, 200, 300, 100 };
	...
	protected virtual void ResetChildPositions ()
	{
		for (int i = 0, imax = mChildren.Count; i < imax; ++i)
		{
			...
			t.localPosition = mHorizontal ? new Vector3(i * ItemSizeArray[i], 0f, 0f) : new Vector3(0f, -i * ItemSizeArray[i], 0f);
			UpdateItem(t, i);
		}
	}
	...
	protected virtual void UpdateItem (Transform item, int index)
	{
		if (onInitializeItem != null)
		{
			int realIndex = (mScroll.movement == UIScrollView.Movement.Vertical) ?
				Mathf.RoundToInt(item.localPosition.y / ItemSizeArray[index]) :
				Mathf.RoundToInt(item.localPosition.x / ItemSizeArray[index]);
